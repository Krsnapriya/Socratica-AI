...
async function buildFullContext({ userId, problemId, sessionId, code, language, executionResult, previousHint }) {
  ...
  return {
    ...
    execution: executionResult ? {
      stdout: executionResult.stdout || (executionResult.results?.[0]?.actualOutput) || "",
      stderr: executionResult.stderr || "",
      error: executionResult.error || (executionResult.verdict === "system_judge_error" ? "system_judge_error" : null),
      exitCode: executionResult.exitCode != null ? executionResult.exitCode : (executionResult.verdict === "pass" ? 0 : 1),
      elapsedMs: executionResult.elapsed_ms || (executionResult.results?.[0]?.elapsed_ms) || 0,
      memoryBytes: executionResult.max_memory_bytes || 0,
      verdict: executionResult.verdict || null,
      testResults: executionResult.results || executionResult.testResults || null,
      passedTestCount: executionResult.passedTests != null ? executionResult.passedTests : (executionResult.passedTestCount || null),
      failedTestCount: executionResult.totalTests && executionResult.passedTests != null
        ? executionResult.totalTests - executionResult.passedTests
        : (executionResult.failedTestCount || null),
      totalTestCount: executionResult.totalTests != null ? executionResult.totalTests : (executionResult.totalTestCount || null),
      allTestsPassed: executionResult.verdict === "pass" || executionResult.allTestsPassed || false,
    } : null,
    testResults,
